...
# Node.js 18 (Vite compatible) and npm installation
...
# Vite build
ENV VITE_ASSET_URL=https://conering.online
RUN npm run build
...
CMD ["sh", "-c", "echo 'APP_KEY:' $APP_KEY && php artisan config:cache && php artisan route:cache && php artisan view:cache && php artisan serve --host=0.0.0.0 --port=9000"]
